...
class _DecksScreenState extends State<DecksScreen> {
  ...
  Widget _buildDecksList() {
    final decks = _deckRepository.decks;
  ...
  Widget _buildDeckCard(Deck deck) {
    ...
    final isValid = validationErrors.isEmpty || (validationErrors.length == 1 && validationErrors.first.contains('Deck size'));
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  if (commander != null)
                    ColorIdentityPips(colors: commander.colorIdentity, size: 18),
                ],
              ),
                ...
                children: [
                  ...
                  Expanded(
                    child: _buildProgressIndicator(
                      ...
                      deck.landCount,
                      deck.landGoal,
                      deck.landCount >= deck.landGoal ? Colors.green : Colors.orange,
                    ),
                  ),
                  const SizedBox(width: 16),
                  Column(
              ...
              if (validationErrors.isNotEmpty) ...[
                ...
                Wrap(
                  spacing: 4,
                  runSpacing: 4,
                  children: validationErrors.take(3).map((error) => ValidationChip(
                    text: error,
  ...
  Widget _buildProgressIndicator(String label, int current, int target, Color color) {
      ...
      MaterialPageRoute(
        builder: (context) => CommanderSelectionScreen(
          onCommanderSelected: (commanderId, strategy) async {
            final commander = _cardRepository.getCardById(commanderId);
            if (commander == null) return;
            
            final deck = _deckRepository.createNewDeck(
              '${commander.name} Deck',
              [commanderId],
              strategy: strategy,
            );
